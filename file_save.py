        ...
        if ws['G1'].value is None:     
            for row in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
                ...
                if spec_column_index is not None:
                    ws.column_dimensions[get_column_letter(spec_column_index)].width = 45
...
def tbd_cost(selected_file):
    ...
    # 미정단가 HTML로 변환
    df_excel_sorted = df_excel_sorted.sort_values(by=['분류'], ascending=True)
    df_excel_sorted = df_excel_sorted.fillna('')
